...
export default function ActivityPage() {
  return <PageShell>
    ...
    <section className="section page-section" aria-labelledby="activity-records-title"><div className="container"><div className="section-heading section-heading--single"><div><Eyebrow>Documented activity</Eyebrow><h2 id="activity-records-title">Scholarship in conversation.</h2></div></div><div className="activity-card-grid">{activities.map((activity) => <a className="activity-card" href={activity.href} target="_blank" rel="noreferrer" key={activity.title}><figure><Image src={activity.image} alt="" fill sizes="(max-width: 820px) 100vw, 50vw" /></figure><div><span>{activity.type}</span><h3>{activity.title}</h3><p>{activity.description}</p><Arrow diagonal /></div></a>)}</div></div></section>
